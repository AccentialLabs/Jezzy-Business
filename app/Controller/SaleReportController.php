...
class SaleReportController extends AppController {
	...
	public function getAllSalesByComission($company){
		...
		$sql = "select * from checkouts inner join offers on offers.id = checkouts.offer_id inner join financial_parameters_results on financial_parameters_results.checkout_id = checkouts.id where commissioned_company_id = {$company['Company'] ['id']};";
