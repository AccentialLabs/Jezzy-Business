...
class DashboardController extends AppController {
	...
	//print_r($this->Session->read('CompanyLoggedIn'));
		...
		$firstLoginWizardCounter = $this->Session->read('firstLoginWizardCounter');
		
		if(empty($firstLoginWizardCounter)){
		
			$firstLoginWizardCounter = 1;
		
		}
		
		$this->set('firstLoginWizardCounter', $firstLoginWizardCounter);
    ...
    public function approveScheduleSolicitation() {
		...
		*/
		...
		//$serviceName = utf8_decode($solicitation[0]['schedules_solicitation']['service_name']);
		...
		$className = utf8_encode($classSubclass[0]['classes']['name']);
		$subClassName = utf8_encode($classSubclass[0]['subclasses']['name']);
        //INSERT SCHEDULE
		...
		$queryGetUser = "SELECT * FROM users WHERE id = {$solicitation[0]['schedules_solicitation']['user_id']};";
		$paramGerUser = array(
            'Service' => array(
                'query' => $queryGetUser
            )
        );
        $capturedUser = $this->AccentialApi->urlRequestToGetData('Services', 'query', $paramGerUser);
		
		if(!empty($capturedUser)){
			$this->enviarEmailConfirmaAgendamento(capturedUser[0]['users']['email'], $classSubclass);
		}
...
// Funcoes para o wizard de first user
    public function companyOpenCloseHour() {
		...
		$this->Session->write('firstLoginWizardCounter', 3);
    }
    ...
    public function companyPassword() {
		...
		$this->Session->write('firstLoginWizardCounter', 2);
...
// Push it !
	...
	**/
	...
	public function getServicesToRealized(){
	   ...
	   $secUs = $_SESSION['SecondaryUserLoggedIn'];
			$sql = "select * from schedules where date <= CURDATE() and time_end < CURTIME() and status = 1 and companie_id = {$company['Company']['id']} and secondary_user_id = {$secUs [0]['secondary_users']['id']} ORDER by date, time_end;";
				$param = array(
					'User' => array(
					'query' => $sql
				)
...
inner join offers on offers.id = checkouts.offer_id
 where services_vouchers.id = {$id};";
	...
	/**
	* Função criada para calculcar dias de uso do modo Trial pelas empresas
	*/
	...
	public function testandoEnvio(){
		
		$this->autoRender = false;
		$this->sendMobileNotification(486, "Agendamento confirmado!");
	...
	public function verifySchedules(){
		...
		$dateSolicitation = $this->request->data['date'];
		$hourSolicitation = $this->request->data['hour'];
		$secondUserSolicitation = $this->request->data['secondaryUserID'];
		
		$sql = "select * from schedules where date = '{$dateSolicitation}' and time_begin = '{$hourSolicitation}' and time_end > '{$hourSolicitation}' and secondary_user_id = {$secondUserSolicitation};";
		$param = array(
            'User' => array(
                'query' => $sql
            )
        );		
       $schedules = $this->AccentialApi->urlRequestToGetData('users', 'query', $param);
	   
	   echo json_encode($schedules);
	
	}
	
	public function verifyClientUserSchedules(){
